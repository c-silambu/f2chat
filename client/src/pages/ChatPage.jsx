...
export const ChatPage = () => {
  const { socket, currentRoom, matchStatus, joinQueue, nextUser } = useSocket();
  const {
    localStream,
    isAudioMuted,
    isVideoDisabled,
  ...
  return (
    <div className="w-full max-w-[1750px] mx-auto px-3 sm:px-6 lg:px-8 py-3 flex-1 flex flex-col gap-3 min-h-[calc(100vh-4.5rem)]">
      
      {/* Camera/Mic Permission Error Banner */}
      {permissionError && (
        <div className="glass-panel p-4 rounded-2xl border border-rose-300 bg-rose-50 text-rose-900 flex flex-col sm:flex-row items-center justify-between gap-4 shadow-sm">
      ...
      {/* Main Video & Chat Grid - Expanded Screen Size */}
      <div className="flex-1 grid grid-cols-1 lg:grid-cols-12 gap-3.5 min-h-[560px]">
        
        {/* Left 8 or 9 Cols: Large Dual Video Screen & Controls */}
        ...
        <div className="lg:col-span-4 xl:col-span-3 h-full min-h-[380px]">
          <ChatBox />
        </div>
